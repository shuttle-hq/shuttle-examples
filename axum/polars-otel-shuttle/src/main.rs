...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !std::path::Path::new(&data_file).exists() {
        ...
        eprintln!(
            "Tip: put the file at ./data/yellow_tripdata_2015-01.csv or set DATA_PATH=<path>"
        );
        return Ok(());
    }
...
async fn run_benchmark(_q: Query<BenchmarkQuery>) -> Result<Json<BenchmarkResult>, StatusCode> {
    ...
    tracing::info!(
        histogram.load_time_ms = 1.2_f64 * 1000.0,
        stage = "load",
        "stage timing"
    );
    tracing::info!(
        histogram.clean_time_ms = 0.8_f64 * 1000.0,
        stage = "clean",
        "stage timing"
    );
    tracing::info!(
        histogram.aggregate_time_ms = 0.4_f64 * 1000.0,
        stage = "aggregate",
        "stage timing"
    );
    tracing::info!(
        histogram.sort_filter_time_ms = 0.3_f64 * 1000.0,
        stage = "sort_filter",
        "stage timing"
    );
    tracing::info!(
        histogram.save_time_ms = 0.1_f64 * 1000.0,
        stage = "save",
        "stage timing"
    );
    tracing::info!(
        histogram.total_time_ms = 2.8_f64 * 1000.0,
        stage = "total",
        "stage timing"
    );
    ...
    tracing::info!(
        monotonic_counter.rows_processed = 12_748_986_i64,
        "rows processed (demo)"
    );
    ...
    tracing::info!(
        histogram.request_duration_ms = elapsed_ms,
        route = "/benchmark",
        "Benchmark responded"
    );
